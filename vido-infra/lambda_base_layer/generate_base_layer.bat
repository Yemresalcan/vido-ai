rem Generates a base layer for the Lambda functions.

rem Remove the container first (if it exists).
docker rm layer-container

rem Build the base layer.
docker build -t base-layer .

rem Rename it to layer-container.
docker run --name layer-container base-layer

rem Copy the generated zip artifact so our CDK can use it.
docker cp layer-container:layer.zip . && echo "Created layer.zip with updated base layer."

pause
